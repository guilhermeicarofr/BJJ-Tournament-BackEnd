...
import { closeEvent, createNewEvent, finishEvent, listCreatorEvents, runEventFights, setFightWinner } from 'services/creator-services';
...
export async function putFightWinner(req: AuthRequest, res: Response, next: NextFunction) {
  const { userId } = req.auth;
  const fightId = Number(req.params.fightId);
  const winnerId = Number(req.body.winnerId);

  try {
    await setFightWinner({ userId, fightId, winnerId });
